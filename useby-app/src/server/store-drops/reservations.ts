...
export function buildReservationPolicyLockSql() {
  return `
    select
      ...
      d.pickup_window_end::text as expires_at,
          ...
          `
            ...
            set
              metadata = coalesce(metadata, '{}'::jsonb) ||
                jsonb_build_object('soldOutAt', coalesce(metadata->>'soldOutAt', now()::text)),
              updated_at = now()
