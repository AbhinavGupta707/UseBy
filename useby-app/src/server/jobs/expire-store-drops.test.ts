...
import { runExpireStoreDropsJob } from "./expire-store-drops";
...
describe("expire store drops job", () => {
  ...
  it("reports unavailable honestly when Aurora env is missing", async () => {
    const result = await runExpireStoreDropsJob({
      source: "/api/jobs/expire-store-drops:test",
    ...
    expect(result.jobType).toBe("expire-store-drops");
  });
});
