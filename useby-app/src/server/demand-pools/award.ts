      ...
      const orderResult = await execTx<{ id: string }>(
        ...
        `
          insert into pool_orders (
            demand_pool_id, commitment_id, household_id, merchant_bid_id,
            merchant_id, merchant_location_id, status, quantity, unit,
            price_cents, currency, pickup_window_start, pickup_window_end,
            status_evidence, metadata, demo_scope_id, is_demo
          )
          select
            c.demand_pool_id,
            c.id,
      ...
      const pickupResult = await execTx<{ id: string }>(
        ...
        `
          insert into pickup_tasks (
            pool_order_id, demand_pool_id, household_id, merchant_id,
            merchant_location_id, status, coarse_pickup_label,
            pickup_window_start, pickup_window_end, evidence, metadata,
