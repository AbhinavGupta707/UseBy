      ...
      const result = await execTx<StoreDropRow>(
        ...
        `
          insert into store_drops (
            ...
            description, category, status, total_quantity, unit, price_cents,
            currency, pickup_window_start, pickup_window_end, available_at,
            expires_at, safety_notes, metadata, demo_scope_id, is_demo
          )
          values (
            :merchantId::uuid, :merchantLocationId::uuid, :neighbourhoodId::uuid,
            :title, nullif(:description, ''), :category, 'draft',
            ...
            nullif(:safetyNotes, ''), :metadata::jsonb, :demoScope, true
          )
          returning
            id::text,
            merchant_id::text,
...
const TRANSITION_POLICY: Record<
  "publish" | "pause" | "close",
  {
    ...
    action: string;
    timestampColumn: "published_at" | "paused_at" | "closed_at";
  }
> = {
  publish: {
  ...
  close: {
    nextStatus: "closed",
    allowed: ["draft", "published", "paused", "sold_out"],
    ...
    return await withTransaction(async (transaction) => {
      ...
      const result = await execTx<StoreDropRow>(
        ...
        `
          ...
          set
            status = :nextStatus,
            ${policy.timestampColumn} = coalesce(${policy.timestampColumn}, now()),
            updated_at = now()
