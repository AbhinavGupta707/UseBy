...
function pickupDto(row: PickupRow) {
  return {
    ...
    pickupWindowStart: row.window_start,
    pickupWindowEnd: row.window_end,
    ...
    const result = await executeSql<PickupRow>({
      sql: `
        select
          ...
          po.price_cents,
          po.currency,
          coalesce(pt.coarse_pickup_hint, po.coarse_pickup_hint) as coarse_pickup_hint,
          pt.window_start::text,
          pt.window_end::text,
      ...
      const order = await execTx<PickupRow>(
        ...
        `
          ...
          where id = :orderId::uuid
            ...
            and status in ('awarded', 'ready', 'pending')
          returning
            id::text as order_id,
        ...
        await execTx(
          ...
          `
            insert into pickup_tasks (
              id, pool_order_id, demand_pool_id, merchant_id, merchant_location_id,
              status, coarse_pickup_hint, ready_at, collected_at, metadata,
              demo_scope_id, is_demo
            )
            values (
              :taskId::uuid, :orderId::uuid, :demandPoolId::uuid,
              :merchantId::uuid, :merchantLocationId::uuid, :status,
              :coarsePickupHint, :readyAt::timestamp with time zone,
          ...
          `,
          {
            ...
            merchantLocationId: context.location.id,
            status,
            coarsePickupHint: row.coarse_pickup_hint ?? context.location.publicAddress,
            readyAt: transition === "ready" ? now : null,
            collectedAt: transition === "collected" ? now : null,
            metadata: { source: "merchant_api", transition, note: input.note ?? null },
            demoScope: context.demoScope,
