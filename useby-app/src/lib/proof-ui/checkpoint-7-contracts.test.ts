...
const CHECKPOINT_7_ROUTE_CONTRACTS: Checkpoint7RouteContract[] = [
  ...
  },
  {
    key: "merchant-edit",
    method: "POST",
    endpoint: "/api/merchant/store-drops/:dropId",
    routeFile: "src/app/api/merchant/store-drops/[dropId]/route.ts",
    requiredInputs: ["merchant actor", "owned draft or paused drop", "title", "quantity", "pickup window"],
    writes: ["store_drops", "audit_events"],
    recomputes: ["merchant-owned drop summary", "reserved capacity from current active reservations"],
    mustReject: ["wrong merchant", "active published drop", "quantity below active reservations", "payment state"],
    mustNotExpose: ["exact household coordinates", "household unit label", "direct personal contact", "payment capture"],
  },
  {
