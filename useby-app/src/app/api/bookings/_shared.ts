...
import {
  ...
  type BookingRuntimeError,
} from "@/server/bookings/runtime";
...
export function runtimeErrorResponse(error: BookingRuntimeError) {
  return NextResponse.json(
    {
      ok: false,
...
export function unknownErrorResponse() {
  return NextResponse.json(
    { ok: false, status: "error", error: "Booking runtime failed." },
    { status: 500 },
  );
}
...
export function bookingCatchResponse(error: unknown) {
